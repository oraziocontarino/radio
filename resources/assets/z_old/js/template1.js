...
var Audio = {
	...
	info:{
		...
		load:function(id,album,artist,title,albumart,mp3){
			...
			albumart=albumart?$("<div>").addClass("albumCoverImage").css("background-image", "url('"+albumart+"')"):''; 
			album=album?album:'Unknown Album';
			title=title?title:'Unknown Title';
            artist=artist?artist:'Unknown Artist';
            $('.album-art').html($("<div>").addClass('albumImageOverlay'));
	...
	player:function(){
		...
		$('.play-list .play').each(function(){
			$(this).on('click',function(e){
				...
				//Audio.play($('.music'));
				$('.music').prop('volume',$('.track-length-bar').val());
                Audio.playlist.hide();
                if($(".play-list").css("display") == "block"){
                    $(".play-list").addClass("hidden");
		...
		$('.track-length-bar').on('change',function(){
			var vol, css;
			vol = $(this).val();
...
function showPlaylist(){
	$(".album-art").addClass("hidden");
	$(".current-info").addClass("hidden");
	$(".play-list").removeClass("hidden");
}

function showTrackInfo(){
	$(".album-art").removeClass("hidden");
	$(".current-info").removeClass("hidden");
	$(".play-list").addClass("hidden");
}