var VlcAPI = (function() {
	...
	function PrivateConstructor() {
		...
		var current_state = STATE["STOP"];
		*/
		function init() {
			console.log("init start");
			server_address = "192.168.1.169";
			baseUrl = "/radio/backend/controller.php";
			//startUpdateStatus();
		...
		function startUpdateStatus(){
			stopUpdateStatus();
			updateStatusInterval = setInterval(function(){
				getStatusPrimise().then(function(data){
					updateStatus(data);
				}).catch(function(data){
					console.log(data);
				});
		...
		*/
		function getStatusAPI(vlcInterfaceStatus)
		{
			return new Promise(function(resolve, reject) {
				resolve(
					$.ajax({
						type: "POST",
						url: baseUrl,
						data: {"api":"status", "vlcInterfaceStatus" : vlcInterfaceStatus}
					})
				);
			 });
        }
...
/*
on document ready fire:

"ajax": {"url": "requests/playlist_jstree.xml"}, "xsl": "nest"
